...
const url = "wss://jyshu0yk1e.execute-api.us-west-1.amazonaws.com/prod";
...
class CanvasDraw extends PureComponent {
  ...
  loadSaveData = (saveData, immediate = this.props.immediateLoading) => {
    ...
    if (
      ...
      this.simulateDrawingLinesWithoutTrigger({
        lines,
        immediate,
      });
    } else {
  ...
  simulateDrawingLinesWithoutTrigger = ({ lines, immediate }) => {
    // Simulate live-drawing of the loaded lines
    // TODO use a generator
    let curTime = 0;
    let timeoutGap = immediate ? 0 : this.props.loadTimeOffset;
    ...
    lines.forEach((line) => {
      ...
      if (immediate) {
        ...
        // Save line with the drawn points
        this.points = points;
        this.saveLineWithoutTrigger({ brushColor, brushRadius });
        return;
      }
  ...
  saveLineWithoutTrigger = ({ brushColor, brushRadius } = {}) => {
    if (this.points.length < 2) return;
...
function onMessage(message) {
    const parsedSavedData = JSON.parse(message.data);
    window.saveableCanvas.loadSaveData(parsedSavedData, true);
}
